...
var DEFAULT_SETTINGS = {
  ...
  latexCommand: 'pdflatex -interaction=nonstopmode -halt-on-error -shell-escape "{input}"; convert -density 150 input.pdf {output}',
  preamble: "% \\input{/path/to/your/preamble.tex}",
    ...
    return __async(this, null, function* () {
      ...
      this.registerMarkdownPostProcessor((el, ctx) => {
        ...
        for (const block of Array.from(codeblocks)) {
          ...
          //if (!first_line.contains("inject-latex"))
          if (first_line.search(/^\`\`\`latex (injection|inject|render|compile|run|load)(\W|$)/)<0)
            continue;
          const payload = lines.slice(1).join("\n");
          el.addClass("latex-render");
          let d;
          el.innerHTML = "";
          d = el;
          d.innerHTML = '<div class="tikz-preview-rendering">Rendering LaTeX...</div>';
    ...
    return new Promise((resolve, reject) => {
      temp.mkdir("obsidian-tikz", (err, dirPath) => {
        ...
        fs.writeFile(inputPath, source, (err2) => {
          if (err2)
            reject(err2);
          const command = this.settings.latexCommand.replace("{input}", "input.tex").replace("{output}", "output.png");
          (0, import_child_process.exec)(command, { cwd: dirPath }, (err3) => {
            if (err3)
              reject(err3);
            fs.readFile(path.join(dirPath, "output.png"), function(err4, data) {
              if (err4)
                reject(err4);
...
var SettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "Settings for LaTeX Injection" });
    new import_obsidian.Setting(containerEl).setName("LaTeX command").setDesc("Command executed to convert LaTeX into PNG").addTextArea((text) => text.setValue(this.plugin.settings.latexCommand).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.latexCommand = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("Preamble").setDesc("Custom preamble").addTextArea((text) => text.setValue(this.plugin.settings.preamble).onChange((value) => __async(this, null, function* () {
